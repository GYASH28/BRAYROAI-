...
expect(files.motion6.includes('@media(prefers-reduced-motion:reduce)'),'Motion V6 reduced-motion safeguards missing');
expect(Buffer.byteLength(files.motion6)<18000,'Motion V6 CSS exceeds 18KB guardrail');
expect(Buffer.byteLength(files.motion6js)<16000,'Motion V6 JS exceeds 16KB guardrail');
...
for(const file of ['public/motion-v5.css','public/motion-v5.js','public/motion-v6.css','public/motion-v6.js','public/terms-page.css','public/terms-page.js']) expect(exists(file),`missing ${file}`);
...
console.log('Integrity OK: dual pricing + Terms + HyperFrames intro + Motion V6 component refinement checked');