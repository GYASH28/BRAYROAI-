...
expect(home.includes('data-v12-story')&&(home.match(/data-v12-step=/g)||[]).length===4,'V12 four-discipline scrolling story is incomplete');
...
for(const token of ['.v12-hero-title','.v12-signal-strip','.v12-story__visual','.v12-project-preview','.v12-ai-products','.v12-product-card','.v12-cursor','.ai-plan-card'])expect(css.includes(token),`V12 CSS missing ${token}`);
for(const token of ['class V12Reveal','class FloatingHeader','class CapabilityStory','class ProjectPreview','class ContextCursor','class ProductTilt','class HeroTextGuard'])expect(js.includes(token),`V12 runtime missing ${token}`);
...
for(const file of ['public/brayro-v12.css','public/brayro-v12.js','public/contact-priority.css','public/contact-priority.js','public/visual-finish.css'])expect(exists(file),`missing ${file}`);
...
console.log('V12 integrity OK: hero, capability story, project showcase, AI offers, pricing and legacy isolation checked');
