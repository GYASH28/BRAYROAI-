...
expect(files.bootstrap.includes('shellCharacter')&&files.bootstrap.includes('data-rae-vector-launcher')&&files.bootstrap.includes('stroke="#ff8a1c"')&&!files.bootstrap.includes('rae-illustration-thumb.webp'),'Rae bootstrap launcher must be a raster-free vector character');
...
for(const token of ['data-rae-rig="v3"','data-rae-vector="full-body"','rae-character__head','rae-character__visor','rae-character__torso','rae-character__pelvis-shell','rae-character__arm--l','rae-character__arm--r','rae-character__hand--l','rae-character__hand--r','rae-character__leg--l','rae-character__leg--r','rae-character__foot-light','rae-character__eyes','rae-character__pupil','rae-character__brows','rae-character__mouth','rae-character__smile-eyes','rae-character__speaking-mouth','rae-character__listening-ring','rae-character__thought'])expect(files.character.includes(token),`Full-vector Rae rig missing ${token}`);
for(const token of ['ensureRaeCharacterSkin','/rae/rae-character-v2.css','linearGradient','radialGradient'])expect(files.character.includes(token),`Rae vector character construction missing ${token}`); expect(!files.character.includes('<image')&&!files.character.includes('/rae/rae-illustration.webp'),'Live Rae character must not use a raster body');
expect(!fs.existsSync('public/rae/rae-illustration.webp')&&!fs.existsSync('public/rae/rae-illustration-thumb.webp'),'Retired Rae raster assets must not ship in public/');
expect(!files.vite.includes('rae-3d')&&!files.pkg.includes('"three"'),'Rae must not load the removed 3D renderer');
...
for(const token of ['role="dialog"','aria-modal="false"','data-rae-stop','data-rae-live','visualViewport','trapFocus','Continue on WhatsApp','rae-card'])expect(files.ui.includes(token),`Rae accessible UI missing ${token}`);
expect(files.ui.includes("this.panel.setAttribute('aria-modal',String(modal))")&&files.ui.includes("this.panel.getAttribute('aria-modal')!=='true'"),'Rae must become modal only on compact viewports and trap focus only while modal');
...
for(const token of ['text/event-stream','AbortController','parsePacket','event:','stream_closed','empty_response','timeoutMs=32000'])expect(files.transport.includes(token),`Rae streaming client missing ${token}`);
...
expect(!/transition\s*:\s*all/i.test(files.css+files.characterCss+files.emotionCss+files.polishCss),'Rae CSS must not use transition: all');
expect(!/backdrop-filter/i.test(files.css+files.characterCss+files.emotionCss+files.polishCss),'Rae must not add backdrop-filter cost');
...
expect(files.api.includes('RAE_PROVIDER')&&files.api.includes('RAE_MODEL')&&files.api.includes('providerCandidates'),'Rae provider abstraction/failover env contract missing');
...
expect(files.api.includes('socialEmotion')&&files.api.includes("PROMPT_VERSION='rae-real-v3'"),'Rae conversational emotion/persona contract missing');
expect(files.api.includes("'gemini-3.8-flash'")&&files.api.includes("'openai/gpt-oss-20b'")&&files.api.includes("code:'all_providers_failed'"),'Rae resilient provider fallback contract missing');
...
expect(Buffer.byteLength(files.polishCss)<8000,'Rae V5 polish CSS exceeds 8KB lazy-load guardrail');
expect(Buffer.byteLength(files.polishJs)<5000,'Rae V5 polish JS exceeds 5KB lazy-load guardrail');
expect(Buffer.byteLength(files.api)<30000,'Rae API exceeds 30KB guardrail');

if(errors.length){console.error(errors.join('\n'));process.exit(1)}
console.log(`Rae integrity OK: ${clientBytes}B modular client, full-vector Rae actor, V4+V5 presentation polish, 21 emotional states, resilient streaming AI, safe actions, collision clearance and verified knowledge.`);
