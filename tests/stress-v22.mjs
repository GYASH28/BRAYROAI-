...
async function waitSecondary(page,selector){await page.waitForSelector('[data-global-nav]',{state:'attached'});if(selector)await page.waitForSelector(selector,{state:'attached'});await waitRaeApp(page)}
...
async function browserLoad(){
  ...
  await page.goto(`${base}/founder`,{waitUntil:'networkidle'});await clearOpening(page);await waitSecondary(page,'[data-principle-stage]');const choices=page.locator('[data-principle]');for(let i=0;i<12;i++)await choices.nth(i%3).click({force:true});assert((await page.locator('[data-principle-stage]').getAttribute('data-sc-verify-state'))==='principle:use','founder principle state');
  await page.goto(`${base}/terms`,{waitUntil:'networkidle'});await waitSecondary(page,'.terms-section');assert(await page.locator('.terms-section').count()===12,'terms section count');assert((await page.locator('body').textContent()).includes('₹9,999'),'terms pricing missing');
