...
export class RaeDirector{
  ...
  setState(next){
    if(!RAE_STATES.includes(next))next='idle';this.state=next;this.lastInteraction=Date.now();this.sleeping=next==='sleep';this.root.dataset.raeState=next;
    ...
    if(next==='celebrate'&&!this.reduced)setTimeout(()=>this.setState('positive'),850);if(['positive','curious','confused','error'].includes(next)&&!this.reduced)setTimeout(()=>{if(this.state===next)this.setState('idle')},1600);
  }
  setEmotion(emotion){const allowed={neutral:'idle',curious:'curious',positive:'positive',thoughtful:'thinking',concerned:'confused'};this.setState(allowed[emotion]||'positive')}
  ...
  trigger(name){if(name==='celebrate')this.setState('celebrate');else if(name==='blink')this.blink();else if(name==='wake')this.wake();else if(name==='sleep')this.sleep()}
  ...
  scheduleBlink(){clearTimeout(this.blinkTimer);if(this.reduced)return;const delay=2500+Math.random()*3000+(Math.random()<.12?2200:0);this.blinkTimer=setTimeout(()=>{if(!document.hidden&&!['thinking','error','sleep'].includes(this.state))this.blink(Math.random()<.12);this.scheduleBlink()},delay)}
  scheduleIdle(){
    clearTimeout(this.idleTimer);if(this.reduced)return;const delay=3600+Math.random()*5200;
    this.idleTimer=setTimeout(()=>{if(document.hidden||this.state!=='idle'){this.scheduleIdle();return}if(Date.now()-this.lastInteraction>65000){this.sleep();return}const pool=[{value:'glance-left',weight:3},{value:'glance-right',weight:3},{value:'tiny-lean',weight:2},{value:'soft-blink',weight:2},{value:'curiosity',weight:1.5},{value:'settle',weight:2}].filter(item=>item.value!==this.lastIdle);const variant=weightedPick(pool);this.lastIdle=variant;this.root.dataset.raeIdle=variant;this.root.style.setProperty('--rae-breath-duration',`${(3.6+Math.random()*1.9).toFixed(2)}s`);if(variant==='soft-blink')this.blink();setTimeout(()=>{if(this.root.dataset.raeIdle===variant)this.root.dataset.raeIdle='neutral'},900+Math.random()*650);this.scheduleIdle()},delay);
  }
  pointer(event){
  ...
  dispose(){clearTimeout(this.idleTimer);clearTimeout(this.blinkTimer);clearTimeout(this.speechTimer);if(this.pointerFrame)cancelAnimationFrame(this.pointerFrame);document.removeEventListener('visibilitychange',this.onVisibility);for(const [name,handler] of this.listeners)document.removeEventListener(name,handler);this.listeners=[]}
}
