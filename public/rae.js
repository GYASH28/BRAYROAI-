(() => {
  ...
  const ensureStylesheet=(href,marker)=>new Promise(resolve=>{
    const current=document.querySelector(`link[${marker}]`);
    if(current){if(current.sheet)return resolve();current.addEventListener('load',resolve,{once:true});current.addEventListener('error',resolve,{once:true});return}
    const link=document.createElement('link');link.rel='stylesheet';link.href=href;link.setAttribute(marker,'');link.addEventListener('load',resolve,{once:true});link.addEventListener('error',resolve,{once:true});document.head.append(link);
  });
  ...
  class RaeBootstrap{
    constructor(){
      ...
      document.body.append(this.root);this.button=this.root.querySelector('[data-rae-shell]');this.bind();
    }
    bind(){
      // Keep initial page work tiny. The full companion is loaded on clear user intent,
      // not by an automatic idle warm-up that can become a long task during first paint.
      ...
      this.button.addEventListener('pointerdown',()=>this.ensure(false),{once:true,passive:true});
    ...
    async ensure(openAfter=false){
      ...
      if(!this.loading){
        ...
        this.loading=Promise.all([ensureStylesheet('/rae.css','data-rae-style'),import('/rae/rae-app.js')]).then(async([,module])=>{
