...
expect(files.v7js.includes("link.href = '/art-direction-v9.css'")&&files.v7js.includes("script.src = '/art-direction-v9.js'"),'Art Direction V9 is not mounted after V8');
for(const token of ['.v9-service-word','.v9-service-state','.v9-proof-reel','.v9-work-caption','.v9-contact-process','.v9-bridge','.v9-founder-note']) expect(files.v9.includes(token),`Art Direction V9 CSS missing ${token}`);
for(const token of ['class HeroContinuityV9','class ServiceStateV9','class WorkProofReelV9','class FounderNoteV9','class ContactFinaleV9','class SceneBridgesV9','class EntrySystemV9','class VelocityBridgeV9']) expect(files.v9js.includes(token),`Art Direction V9 runtime missing ${token}`);
for(const token of ['REAL INTERFACE DETAILS / TAP TO FOCUS','Desktop browsing','Mobile hierarchy','Project decisions','LIVE PROJECT / <b>FAKHRIMART</b>','EMAIL OPENS WITH YOUR SELECTED PROJECT TYPE']) expect(files.v9js.includes(token),`V9 continuity/client proof missing ${token}`);
expect(files.v9.includes('translate:0 var(--v8-word-y)!important')&&files.v9js.includes("--v8-word-y"),'V9 browser-safe velocity bridge missing');
expect(!files.v9.includes('var(--v9-service-p)) *'),'V9 contains unsupported CSS variable multiplication');
...
expect(Buffer.byteLength(files.v9)<18000,'Art Direction V9 CSS exceeds 18KB guardrail');
expect(Buffer.byteLength(files.v9js)<16000,'Art Direction V9 JS exceeds 16KB guardrail');
...
for(const file of ['public/motion-v5.css','public/motion-v5.js','public/motion-v6.css','public/motion-v6.js','public/art-direction-v7.css','public/art-direction-v7-contrast.css','public/art-direction-v7.js','public/work-showcase-v8.css','public/work-showcase-v8.js','public/art-direction-v9.css','public/art-direction-v9.js','public/terms-page.css','public/terms-page.js']) expect(exists(file),`missing ${file}`);
...
console.log('Integrity OK: dual pricing + Terms + HyperFrames + V7 + V8 + Art Direction V9 checked');