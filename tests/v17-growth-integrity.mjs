...
expect(home.includes('Turn missed enquiries')&&home.includes('BRAYRO GROWTH ENGINE'),'Homepage does not lead with the new growth-system positioning');
expect(home.includes('data-ig-demo')&&home.includes('data-ig-calculator'),'Homepage is missing the interactive Growth Engine demo or opportunity calculator');
...
expect(home.includes('FEATURED CLIENT / FAKHRIMART')&&home.includes('Verified client work'),'FakhriMart proof is not clearly labeled as real client work');
expect(home.includes('Built in India')&&home.includes('Pune, India'),'International location honesty is missing');
expect(!home.includes('₹2,599')&&!home.includes('₹9,999 audit fee'),'Retired low-ticket homepage pricing leaked into the new commercial funnel');
expect(!home.includes('world-class solutions')&&!home.includes('10x your growth'),'Homepage contains prohibited generic growth hype');
...
expect(vite.includes("growthAudit:resolve(process.cwd(),'audit.html')"),'Vite input missing Growth Audit page');
for(const token of ["us:resolve(process.cwd(),'us.html')","uae:resolve(process.cwd(),'uae.html')","lab:resolve(process.cwd(),'lab.html')"])expect(vite.includes(token),`Vite input missing ${token}`);
expect(vite.includes('data-scene="growth-services"'),'Legacy capability mutation isolation is missing from the Vite transform');
for(const route of ['/audit','/us','/uae','/lab'])expect(vercel.includes(route),`Vercel missing ${route} route`);
expect(vercel.includes('international-growth.css')&&vercel.includes('international-growth.js'),'Vercel asset cache policy missing new growth assets');

expect(pkg.includes('"version": "17.0.0"'),'Package version is not V17');
expect(pkg.includes('node --check public/international-growth.js'),'Syntax suite does not check the growth runtime');
expect(pkg.includes('node tests/v17-growth-integrity.mjs'),'Integrity suite does not enforce V17 growth contracts');
...
console.log('V17 growth integrity OK: international positioning, honest proof, market routes, pricing, audit funnel and legacy mutation isolation verified.');
