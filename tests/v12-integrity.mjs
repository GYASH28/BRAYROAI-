...
const vercel=exists('vercel.json')?JSON.parse(read('vercel.json')):{rewrites:[],redirects:[]};
const isV21=exists('tests/v21-growth-integrity.mjs');
const errors=[];
const expect=(condition,message)=>{if(!condition)errors.push(message)};

if(isV21){
  expect(home.includes('class="v12-hero-title"')&&home.includes('Turn missed enquiries'),'V21 must preserve V12 hero typography while using the growth promise');
  expect(home.includes('data-v12-story')&&(home.match(/data-v12-step=/g)||[]).length===4,'V21 four-part solution source markup is incomplete');
  expect((home.match(/data-v12-project data-preview=/g)||[]).length===3,'V21 project showcase must contain three previewable rows');
  ...
  for(const text of ['BRAYRO GROWTH ENGINE','AI OPERATIONS','CUSTOM DIGITAL SYSTEMS','BRAYRO OS'])expect(home.includes(text),`V21 homepage solution architecture missing ${text}`);
  for(const price of ['From US$1,500','From US$2,500','US$3K–$15K+'])expect(home.includes(price),`V21 homepage missing commercial price signal ${price}`);
  expect(home.includes('data-ig-demo')&&home.includes('data-ig-calculator'),'V21 homepage must keep useful interactive commercial proof');
  expect(home.includes('/brayro-v12.css')&&home.includes('/brayro-v12.js'),'V12 assets are not mounted on V21 homepage');

  expect(plans.includes('Pay for a connected outcome'),'V21 pricing page must be outcome-led');
  expect((plans.match(/class="ig-offer/g)||[]).length>=3,'V21 plans must retain detailed offer cards');
  for(const text of ['Growth Engine','AI Operations','Custom Digital Systems','From US$1,500','From US$2,500','US$3,000–$15,000+'])expect(plans.includes(text),`V21 plans missing ${text}`);
  expect(plans.includes('WHAT CHANGES THE PRICE?')&&plans.includes('Third-party costs'),'V21 pricing scope boundaries are not clear enough');
  expect(!plans.includes('₹2,599')&&!plans.includes('₹17,999'),'retired low-ticket pricing leaked into V21 plans');
}else{
...
expect(pkg.includes('"version": "16.0.0"'),'package release compatibility is not V16');
...
const routed=new Set([...(vercel.rewrites||[]),...(vercel.redirects||[])].map(rule=>String(rule.source||'').replace(/^\//,'')));
for(const [name,html] of Object.entries({home,plans})){
  const refs=[...html.matchAll(/(?:src|href)="\/(?!\/)([^"?#]+)["?#]?/g)].map(match=>match[1]).filter(Boolean);
  for(const ref of refs){
    const valid=[path.join(root,'public',ref),path.join(root,ref),path.join(root,`${ref}.html`)].some(fs.existsSync)||routed.has(ref);
    expect(valid,`missing local ${name} reference: /${ref}`);
  }
}

if(errors.length){console.error(errors.join('\n'));process.exit(1)}
console.log('V12 compatibility integrity OK beneath V21: visual runtime, proof, commercial detail and route resolution checked');
