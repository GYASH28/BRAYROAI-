...
triggers.forEach(button=>{button.firstChild.textContent=arabic?'الإمارات · العربية ':market==='in'?'Choose market':label+' '});
...
dialog.innerHTML=`<div class="market-sheet__head"><span>${arabic?'BRAYROAI / اختر السوق':'BRAYROAI / MARKET MODE'}</span><button type="button" data-market-close aria-label="${arabic?'إغلاق اختيار السوق':'Close market choices'}">×</button></div><h2>${arabic?'أين تبني مشروعك؟':'Where are you building?'}</h2><p>${arabic?'اختر السوق الذي تريد الأسعار والعرض له. سيبقى اختيارك في جميع الصفحات.':'Choose the market you want prices and proposals for. Your choice follows every page.'}</p><div class="market-sheet__options"><a data-market-choice="ae" href="${marketRoute('ae',route,location.hash)}"><strong>${arabic?'الإمارات العربية المتحدة':'United Arab Emirates'}</strong><span>AED · English</span></a><a data-market-choice="au" href="${marketRoute('au',route,location.hash)}"><strong>${arabic?'أستراليا':'Australia'}</strong><span>A$ · English</span></a></div><div class="market-sheet__language" data-market-language><span>${arabic?'لغة موقع الإمارات':'UAE language'}</span><a data-market-choice="ae" href="${marketRoute('ae',route,location.hash)}">English</a><a data-market-choice="ae-ar" href="${marketRoute('ae-ar',route,location.hash)}">العربية</a></div><small>${arabic?'لا نخمّن موقعك ولا نحوّل الأسعار آلياً. يعمل BRAYROAI عن بُعد من بونه، الهند.':'No location guessing or automatic currency conversion. BRAYROAI works remotely from Pune, India.'}</small>`;
