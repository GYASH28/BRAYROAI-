...
const routes=['/','/plans','/founder','/terms','/commercial-cut.css','/commercial-cut.js','/direction-pass.css','/direction-pass.js','/motion-v4.css','/motion-v5.css','/motion-v5.js','/motion-v6.css','/motion-v6.js','/art-direction-v7.css','/art-direction-v7-contrast.css','/art-direction-v7.js','/work-showcase-v8.css','/work-showcase-v8.js','/art-direction-v9.css','/art-direction-v9.js','/continuity-v10.css','/continuity-v10.js','/useful-ux-v11.css','/useful-ux-v11.js','/plans-page.css','/plans-page.js','/founder-page.css','/founder-page.js','/terms-page.css','/terms-page.js','/scrollcraft.css','/scrollcraft.js','/assets/hero-background.webp','/assets/yash-cutout.webp','/assets/about-yash.webp','/assets/fakhrimart-case-desktop.png','/assets/fakhrimart-case-mobile.png','/assets/brayroai-cinematic-opening.mp4'];
...
async function waitV11(page){await page.waitForSelector('link[data-useful-ux-v11]',{state:'attached'});await page.waitForSelector('script[data-useful-ux-v11]',{state:'attached'});await page.waitForFunction(()=>document.body.classList.contains('v11-ready'));}
...
async function browserLoad(){
  ...
  for(const vp of [{width:390,height:844},{width:1440,height:900},{width:1920,height:1080}]){
    await page.setViewportSize(vp);await page.goto(`${base}/`,{waitUntil:'networkidle'});await clear(page);await waitV11(page);
    assert(await page.locator('[data-scene]').count()===7,`home scenes @${vp.width}`);
    assert((await page.locator('#services').getAttribute('data-scene'))==='outcomes',`old second scene returned @${vp.width}`);
    assert(await page.locator('[data-v11-outcome]').count()===3,`useful outcome controls missing @${vp.width}`);
    assert(await page.locator('[data-capability-stage]').count()===0,`old capability instrument still visible @${vp.width}`);
    ...
    assert(await page.locator('.v9-contact-process>div').count()===3,'contact process missing');
    await scrollStorm(page,2,18);
  }

  await page.setViewportSize({width:1440,height:900});
  await page.goto(`${base}/`,{waitUntil:'networkidle'});await clear(page);await waitV11(page);
  for(let i=0;i<90;i++){
    const keys=['clarity','enquiries','system'];
    await page.locator(`[data-v11-outcome="${keys[i%3]}"]`).click();
  }
  assert((await page.locator('#services').getAttribute('data-sc-verify-state'))==='outcome:system','outcome selector lost final state');
  ...
  await page.goto(`${base}/plans`,{waitUntil:'networkidle'});await clear(page);await waitV11(page);
  ...
  await page.goto(`${base}/founder`,{waitUntil:'networkidle'});await clear(page);await waitV11(page);
  ...
  await page.goto(`${base}/terms`,{waitUntil:'networkidle'});await waitV11(page);
