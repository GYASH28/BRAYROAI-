...
expect(!/canvas|webgl|three\.js|requestAnimationFrame\([^)]*render/i.test(files.character+files.characterCss),'Detailed Rae must remain lightweight SVG/CSS, not a permanent render loop');
...
expect(!/transition\s*:\s*all/i.test(files.css+files.characterCss),'Rae CSS must not use transition: all');
expect(!/backdrop-filter/i.test(files.css+files.characterCss),'Rae must not add backdrop-filter cost');
...
expect(clientBytes<115000,`Rae modular JS exceeds 115KB guardrail (${clientBytes})`);
...
console.log(`Rae integrity OK: ${clientBytes}B modular client, detailed V3 ivory/visor character rig, emotional motion, real streaming AI, safe actions, contact-dock clearance and verified knowledge.`);
